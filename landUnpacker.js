...
import { ADDRESS_FOR_TRANSFER } from "../../constants/contract.constants";
...
export default function LandUnpacker() {
  ...
  useEffect(() => {
    if (user.addr) fetchUserPacksIds();
  }, [user]);
  ...
  const fetchUserPacks = (ids) => {
    if (ids.length)
      axios
        .get(`${MARKET_NFT_API}/user-nfts?itemIds=${ids.join(",")}`)
    ...
    items
      ...
      .forEach((item) => {
        const foundItem = result.find(
          ({ item_template_id }) => item.item_template_id === item_template_id
        );
        ...
        if (foundItem) foundItem.amount++;
          ...
          result.push({
            ...item,
            amount: 1,
            image: getImageByPackTemplateId(item.item_template_id),
          });
      });
  ...
  //   timestamp = 1667769807797 for bonus pack 6/11/2022
  ...
  const fetchUnpackedLands = async () => {
    const { data } = await axios.get(
      `${FLOW_UNPACKER_API}?userAddress=${user.addr}&unpackedTimestamp=${unpackedTimestamp}`
    );
